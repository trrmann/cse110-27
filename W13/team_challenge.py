...
def compute_area_rectangle(length:float, width:float):
    return (length * width)

def compute_area_circle(radius:float):
    return math.pi * (radius ** 2)

def compute_area(shape:str, first_dimenstion:float, second_dimenstion:float = None):
    match shape:
        case "square":
            return compute_area_rectangle(first_dimenstion, first_dimenstion)
        case "rectangle":
            return compute_area_rectangle(first_dimenstion, second_dimenstion)
        case "circle":
            return compute_area_circle(first_dimenstion)
        case _:
            return None

def request_square(square:str):
    square_side = request_positive_float("Please enter the length of a side of the square: ", 0)
    print(f"The area of the square with a length of {square_side} is {compute_area(square, square_side):.2f}")

def request_rectangle(rectangle:str):
    ...
    print(f"The area of the rectangle with a length of {rectangle_length} and width of {rectangle_width} is {compute_area(rectangle, rectangle_length, rectangle_width):.2f}")

def request_circle(circle:str):
    circle_radius = request_positive_float("Please enter the radius of a circle: ", 0)
    print(f"The area of the circle with a radius of {circle_radius} is {compute_area(circle, circle_radius):.2f}")
...
def calculate_area():
    ...
    while reply != "quit":
        try:
            ...
            match reply:
                case "square":
                    request_square(reply)
                case "rectangle":
                    request_rectangle(reply)
                case "circle":
                    request_circle(reply)
                case _:
