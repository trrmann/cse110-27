...
class Wordle():
    ...
    __track_key__ = "track"
    __lost_key__ = "lost"
    __data_filename_key__ = "dataFilename"
    __data_filename_access_expire_age_days_key__ = "dataFilenameAccessExpireAgeDays"
    __data_filename_modify_expire_age_days_key__ = "dataFilenameModifyExpireAgeDays"
    __data_filename_create_expire_age_days_key__ = "dataFilenameCreateExpireAgeDays"
    __online_key__ = "online"
    __letters_key__ = "letters"
    __non_case_words_key__ = "non_case_words"
    __count_key__ = "count"
    __percent_key__ = "percent"
    __rank_key__ = "rank"
    __data__ = None
    __default_list__ = ["dog", "cat", "pig", "cow", "ape", "sow", "sheep", "sleep", "house", "horse", "apple"]
    ...
    def __repr__(self) -> str:
        ...
        string = f"{string}, \"{self.__data_filename_access_expire_age_days_key__}\" : {self.get_data_filename_access_expire_age_days()}"
        string = f"{string}, \"{self.__data_filename_modify_expire_age_days_key__}\" : {self.get_data_filename_modify_expire_age_days()}"
        string = f"{string}, \"{self.__data_filename_create_expire_age_days_key__}\" : {self.get_data_filename_create_expire_age_days()}"
        string = f"{string}, \"{self.__data_filename_key__}\" : \"{self.get_data_filename()}\""
        string = f"{string}, \"{self.__word_list_key__} count\" : {len(self.get_word_list())}"
    ...
    def purify_alpha(self, string, min_len = 2):
        if len(string) < min_len:
            ...
            if self.isAlpha(string[0]) and self.isAlpha(string[len(string)-1]):
                ...
                return self.purify_alpha(string[0:len(string)-1], min_len)
            elif self.isAlpha(string[len(string)-1]):
                return self.purify_alpha(string[1:len(string)], min_len)
            else:
                return self.purify_alpha(string[0:len(string)-1], min_len)
    ...
    def read_bible_strings_from_github(self):
        data = {}
        data[self.__online_key__] = False
        if self.test_connection_to_github():
            data[self.__online_key__] = True
            ...
            for letter in letters.keys():
                ...
                letters[letter] = {}
                letters[letter][self.__count_key__] = count
                letters[letter][self.__percent_key__] = letters[letter][self.__count_key__] / letter_count * 100
                by_per[letters[letter][self.__percent_key__]] = letter
            pk = [*by_per.keys()]
            ...
            for letter in letters.keys():
                letters[letter][self.__rank_key__] = rnk[letter]
            word_count = 0
            ...
            for word in non_case_words.keys():
                ...
                non_case_words[word] = {}
                non_case_words[word][self.__count_key__] = count
                non_case_words[word][self.__percent_key__] = non_case_words[word][self.__count_key__] / word_count * 100
                by_per[non_case_words[word][self.__percent_key__]] = word
            pk = [*by_per.keys()]
            pk.sort(reverse=True)
            rnk = {}
            for idx in range(len(pk)):
            ...
            for word in non_case_words.keys():
                if word in rnk.keys(): non_case_words[word][self.__rank_key__] = rnk[word]
                else:  non_case_words[word][self.__rank_key__] = rnk[by_per[non_case_words[word][self.__percent_key__]]]
            ...
            data[self.__letters_key__] = letters
            data[self.__non_case_words_key__] = non_case_words
            data[self.__word_list_key__] = words
        else:
            data[self.__word_list_key__] = {}
        return data

    def get_default_datafile_data(self):
        default_data = {}
        word_list = {}
        for index in range(len(self.__default_list__)):
            word = self.__default_list__[index]
            word_list[word] = {}
            word_list[word][self.__count_key__] = 1
            word_list[word][self.__rank_key__] = 1
    ...
    def build_file(self, filename :str = None):
        if filename == None: filename = self.get_data_filename()
        github_data = self.read_bible_strings_from_github()
        if github_data != {}:
            self.create_empty_file(filename)
            if not(self.__word_list_key__ in github_data.keys()):
                self.write_data_to_file(filename, self.get_default_datafile_data())
            else:
                self.write_data_to_file(filename, github_data)
    ...
    def set_data_filename_access_expire_age_days(self, access_expire_age_days :int = 3):
        self.__data__[self.__data_filename_access_expire_age_days_key__] = int(access_expire_age_days)
        return self

    def get_data_filename_access_expire_age_days(self, access_expire_age_days :int = None):
        if self.__data_filename_access_expire_age_days_key__ in self.__data__.keys(): return self.__data__[self.__data_filename_access_expire_age_days_key__]
    ...
    def set_data_filename_modify_expire_age_days(self, modify_expire_age_days :int = 7):
        self.__data__[self.__data_filename_modify_expire_age_days_key__] = int(modify_expire_age_days)
        return self

    def get_data_filename_modify_expire_age_days(self, modify_expire_age_days :int = None):
        if self.__data_filename_modify_expire_age_days_key__ in self.__data__.keys(): return self.__data__[self.__data_filename_modify_expire_age_days_key__]
    ...
    def set_data_filename_create_expire_age_days(self, create_expire_age_days :int = 30):
        self.__data__[self.__data_filename_create_expire_age_days_key__] = int(create_expire_age_days)
        return self

    def get_data_filename_create_expire_age_days(self, create_expire_age_days :int = None):
        if self.__data_filename_create_expire_age_days_key__ in self.__data__.keys(): return self.__data__[self.__data_filename_create_expire_age_days_key__]
    ...
    def set_data_filename(self, data_filename :str = "data.dat"):
        self.__data__[self.__data_filename_key__] = str(data_filename)
        return self

    def get_data_filename(self, data_filename :str = None):
        if self.__data_filename_key__ in self.__data__.keys(): return self.__data__[self.__data_filename_key__]
