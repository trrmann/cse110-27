...
class Wordle():
    ...
    def is_next_turn(self):
        return self.does_guess_not_equal_secret_word() and\
            ...
            ((self.get_max_turns() >= 1) and (self.get_turn_number() < self.get_max_turns())))
    ...
    def configure_game(self):
        self.set_number_of_players(int(input("How many players? ")))
        self.set_max_turns(int(input("How many guesses do you want? ")))
        self.set_super_hint_frequency(0)
        if str(input("Do you want super hints [(y)es/(n)o]? ")).lower() in ["y", "yes", empty]:
            if self.get_max_turns() > 0:
                ...
                if super_hints >= self.get_max_turns(): self.set_super_hint_frequency(1)
                elif super_hints > 0: self.set_super_hint_frequency(int(self.get_max_turns() / super_hints))
            else: self.set_super_hint_frequency(int(input("How often do you want super hints? ")))

    def play_turn(self):
        if self.get_number_of_players() > 1:
            if self.get_multiplayer_init(): input("Press enter to continue.")
        ...
        self.set_guess(input("what is your guess? "))
    ...
    def play_game(self):
        ...
        self.set_multiplayer_init(self.set_turn_number())
        ...
        if(self.get_secret_word() == self.get_guess()): self.game_win_message()
